...
def get_explainer(model, X1_train, X2_train, y_train, r=0.5):
    # get abnoraml data
    X1_train,X2_train = SelectAbnormal(X1_train,X2_train,y_train)
    np.random.seed(42)
    sample_size = int(X1_train.shape[0] * r)
    sample_indices = np.random.choice(X1_train.shape[0], size=sample_size, replace=False)

    ##### train identifier
    ### kernel

    KX1_train = X1_train[sample_indices]
    explainer1 = shap.KernelExplainer(model.predict_ts, KX1_train, link="logit")

    ### Gradient
    GX2_train = X2_train[sample_indices]
    n = GX2_train.shape[0]
    GX1_train = np.tile(model.X1, (n, 1))  # construct a filled normal numerical time series matrix
...
def pretrain(model, dataset, X1_train, X2_train, y_train, n = None):
    ...
    #get explainer and abnormal data 
